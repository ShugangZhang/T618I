...
int main(int argc, char *argv[])
{
	...
	ToRORD* strand[cellNum]; // note that constructor contains the initializer
	...
	for (int i = 0; i < cellNum; i++)
	{
		...
		if(i >= 0 && i < endoCellNum)
		{
			...
			// read in initial values (this is because the original init values is not stable yet)
			// if the initfile is not available, run the initialization.cc first
			initfile = fopen("SingleCell/ToRORDInitialValues_CON_ENDO.dat","r");
			strand[i]->readinAllStates(initfile);   // must be uncommented to get 0.719 m/s
			fclose(initfile);
		}
		else if (i < endoCellNum + mCellNum)
		{
			...
			// read in initial values (this is because the original init values is not stable yet)
			// if the initfile is not available, run the initialization.cc first
			initfile = fopen("SingleCell/ToRORDInitialValues_CON_MCELL.dat","r");
			// strand[i]->readinAllStates(initfile);   // must be uncommented to get 0.719 m/s
			fclose(initfile);
		}
		else // i < total cellnum
		{
			...
			// read in initial values (this is because the original init values is not stable yet)
			// if the initfile is not available, run the initialization.cc first
			initfile = fopen("SingleCell/ToRORDInitialValues_CON_EPI.dat","r");
			// strand[i]->readinAllStates(initfile);   // must be uncommented to get 0.719 m/s
			fclose(initfile);
		}	
		#endif
		...
		if(i >= 0 && i < endoCellNum)
		{
			...
			// read in initial values (this is because the original init values is not stable yet)
			// if the initfile is not available, run the initialization.cc first
			// initfile = fopen("SingleCell/ToRORDInitialValues_QUIN_ENDO.dat","r");
			// strand[i]->readinAllStates(initfile);   // must be uncommented to get 0.719 m/s
			// fclose(initfile);
		}
		else if (i < endoCellNum + mCellNum)
		{
			...
			// read in initial values (this is because the original init values is not stable yet)
			// if the initfile is not available, run the initialization.cc first
			// initfile = fopen("SingleCell/ToRORDInitialValues_QUIN_MCELL.dat","r");
			// strand[i]->readinAllStates(initfile);   // must be uncommented to get 0.719 m/s
			// fclose(initfile);
		}
		else // i < total cellnum
		{
			...
			// read in initial values (this is because the original init values is not stable yet)
			// if the initfile is not available, run the initialization.cc first
			// initfile = fopen("SingleCell/ToRORDInitialValues_QUIN_EPI.dat","r");
			// strand[i]->readinAllStates(initfile);   // must be uncommented to get 0.719 m/s
			// fclose(initfile);
		}	
		#endif
	...
	#ifdef CON
	FILE *datafile = fopen("Outputs/VentOneDResults_CON1.dat","w+");
	#endif
	...
	#ifdef SQT1
	FILE *datafile = fopen("Outputs/VentOneDResults_SQT1.dat","w+");
	#endif

	#ifdef QUIN
	FILE *datafile = fopen("Outputs/VentOneDResults_QUIN.dat","w+");
	#endif
